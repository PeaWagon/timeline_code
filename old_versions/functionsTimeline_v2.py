...
class Timeline(object):
    ...
    colours = ['teal','magenta','yellow','gold','red','blue','grey']
    ProP_residues_let = ['EHYDNI EQKIDDIDHE IADLQAKRTR LVQQHPRIDE']
        ...
        while sr_c.count(segment) > 0:
            ...
            if sr_c[rind].count(num) == 1:
                ...
                if self.PCT == True:
                    if num > 36:
                        num-=36
                    residue = self.ProP_res[num]
                    for key in self.res_desc:
                        if residue in self.res_desc[key]:
                            desc = key 
                    print('Residue type is '+residue+', which is '+desc+'.')
                print()
                return True
            else:
                sr_c.remove(segment)
    ...
    def res_stats(self, full_id):
        ...
        print()
        qs = input("Type \"apple\" if you want a turtle picta. ")
        if qs == 'apple':
            self.turtle_picta(full_id, res_frames)
        print()
